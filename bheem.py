...
class BheemParser:
    ...
    def parse_alert_message_data(self, message: str) -> dict:
        ...
        if message.lower().find("sl") >= 0:
            self.alert["value"] = "be"
            if message.lower().find("updated to h") >= 0 or message.lower().find("stop updated h") >= 0:
        ...
        elif message.lower().find("close") >= 0 or message.lower().find("booked") >= 0 or message.lower().find("take profit") >= 0:
    ...
    def parse_trade_message_data(self, message: str) -> dict:
        """parse bheem message"""
        # remove server tag
        message = message.replace(f'<@&1212853382549872711>','').strip().strip('**')
        lines = message.split("\n")
        if len(lines) == 1:
            # one line message
